...
public class TestCorsFilter {
    ...
    public void testDoFilterSimpleGET() throws Exception {
        ...
        HttpGet httpGet = new HttpGet("http://localhost:8080/user/params");
        httpGet.setHeader(CorsFilter.REQUEST_HEADER_ORIGIN, TesterFilterConfigs.HTTPS_WWW_APACHE_ORG);
        httpGet.setHeader("xxx", null);
        CloseableHttpResponse response = httpClient.execute(httpGet);

        Assert.assertTrue(response.containsHeader(CorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN));
        Assert.assertTrue(response.getHeaders(CorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN).equals("https://www.apache.org"));
     ...
     */
    @Test
    public void testDoFilterSimpleAnyOriginAndSupportsCredentials() throws IOException, ServletException {
        TesterHttpServletRequest request = new TesterHttpServletRequest();
        request.setHeader(CorsFilter.REQUEST_HEADER_ORIGIN, TesterFilterConfigs.HTTPS_WWW_APACHE_ORG);
    ...
    public void closeQuietly(CloseableHttpClient client, CloseableHttpResponse response) {
        try {
            response.close();
            client.close();
        } catch (Exception e) {
